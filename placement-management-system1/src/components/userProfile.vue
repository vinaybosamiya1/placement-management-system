            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              ...
              <div class="flex flex-wrap justify-center gap-3">
                ...
                <router-link to="/editprofile">
                  <button class="cursor-pointer inline-flex items-center gap-2 px-5 py-2.5 text-xs font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 rounded-xl shadow-lg shadow-blue-500/25 transition-all active:scale-95">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="space-y-6">
              ...
              <div class="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl">
                ...
                <div class="border-t border-slate-800/80 pt-4 space-y-2.5">
                  <a :href="student.socials.linkedin" target="_blank" class="flex items-center gap-3 text-xs font-semibold text-slate-300 hover:text-blue-400 transition-colors p-2 rounded-xl bg-slate-950 border border-slate-800">
                    <svg class="w-4 h-4 text-blue-400 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.779-1.75-1.75s.784-1.75 1.75-1.75 1.75.779 1.75 1.75-.784 1.75-1.75 1.75zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                    <span>LinkedIn Profile</span>
                  </a>
                  <a :href="student.socials.github" target="_blank" class="flex items-center gap-3 text-xs font-semibold text-slate-300 hover:text-white transition-colors p-2 rounded-xl bg-slate-950 border border-slate-800">
                    <svg class="w-4 h-4 text-slate-400 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.137 20.162 22 16.418 22 12c0-5.523-4.477-10-10-10z"/></svg>
              ...
              <div class="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                ...
                <div class="flex flex-wrap gap-2.5 pt-2">
                  <span v-for="(skill, index) in student.skills" :key="index"
                        class="px-3.5 py-2 bg-slate-950 border border-slate-800 text-blue-300 rounded-xl text-xs font-semibold hover:border-blue-500/40 hover:text-white transition-all shadow-sm">
                    ⚡ {{ skill }}
                  </span>
                </div>
              </div>
...
const downloadResume = () => {
  alert('Downloading student resume PDF...');
};
