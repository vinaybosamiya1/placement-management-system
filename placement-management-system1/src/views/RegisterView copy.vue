...
const focus = reactive({ firstName: false, lastName: false, email: false, password: false, confirm: false });
...
onMounted(() => {
  particles.value = Array.from({ length: 18 }, (_, i) => ({
    id: i,
    style: `left:${Math.random()*100}%;top:${Math.random()*100}%;width:${6+Math.random()*10}px;height:${6+Math.random()*10}px;animation-delay:${Math.random()*6}s;animation-duration:${6+Math.random()*8}s;opacity:${0.15+Math.random()*0.25};`,
  }));
});
...
<style scoped>
.blob { position:absolute;border-radius:50%;filter:blur(80px);opacity:0.4;animation:blob-float 12s ease-in-out infinite alternate; }
.blob-1 { width:500px;height:500px;background:#1e40af;top:-150px;left:-150px;animation-delay:0s; }
.blob-2 { width:400px;height:400px;background:#3b82f6;bottom:-100px;right:-100px;animation-delay:3s; }
.blob-3 { width:300px;height:300px;background:#60a5fa;top:50%;left:50%;animation:blob-float3 10s ease-in-out infinite alternate; }
@keyframes blob-float { 0%{transform:scale(1) translate(0,0)}100%{transform:scale(1.15) translate(30px,30px)} }
@keyframes blob-float3 { 0%{transform:translate(-50%,-50%) scale(1)}100%{transform:translate(-40%,-60%) scale(1.2)} }

.particle { position:absolute;background:rgba(255,255,255,0.6);border-radius:50%;animation:particle-rise linear infinite; }
@keyframes particle-rise { 0%{transform:translateY(0) scale(1);opacity:0}10%{opacity:1}90%{opacity:1}100%{transform:translateY(-100vh) scale(0.5);opacity:0} }

.glass-card { background:rgba(255,255,255,0.92);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.5); }

.icon-ring { width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#2563eb,#1d4ed8);display:flex;align-items:center;justify-content:center;box-shadow:0 8px 32px rgba(37,99,235,0.4);animation:icon-pulse 3s ease-in-out infinite;position:relative; }
.icon-ring::before { content:'';position:absolute;inset:-4px;border-radius:50%;border:2px solid rgba(37,99,235,0.3);animation:ring-spin 4s linear infinite; }
@keyframes icon-pulse { 0%,100%{box-shadow:0 8px 32px rgba(37,99,235,0.4);transform:scale(1)}50%{box-shadow:0 12px 48px rgba(37,99,235,0.6);transform:scale(1.04)} }
@keyframes ring-spin { from{transform:rotate(0deg)}to{transform:rotate(360deg)} }

.step-dot { width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:700;background:#e5e7eb;color:#9ca3af;transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1);flex-shrink:0; }
.step-dot.active { background:#2563eb;color:white;transform:scale(1.1);box-shadow:0 4px 12px rgba(37,99,235,0.4); }
.step-dot.done { background:#16a34a;color:white; }
.step-line { width:40px;height:2px;background:#e5e7eb;border-radius:2px;transition:background 0.4s;flex-shrink:0; }
.step-line.active { background:#2563eb; }

.form-group { position:relative; }
.float-label { position:absolute;left:40px;top:13px;font-size:0.875rem;color:#9ca3af;transition:all 0.25s cubic-bezier(0.4,0,0.2,1);pointer-events:none;z-index:1; }
.form-group.focused .float-label, .form-group.filled .float-label { top:-8px;left:36px;font-size:0.72rem;color:#2563eb;background:white;padding:0 4px;font-weight:600; }
.form-group.error .float-label { color:#dc2626 !important; }
.input-wrap { position:relative; }
.input-icon { position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#9ca3af;transition:color 0.2s;display:flex; }
.form-group.focused .input-icon { color:#2563eb; }
.form-group.error .input-icon { color:#dc2626; }
.float-input { width:100%;padding:12px 12px 12px 40px;border:2px solid #e5e7eb;border-radius:12px;font-size:0.9rem;outline:none;background:#f9fafb;transition:border-color 0.25s,background 0.25s,box-shadow 0.25s; }
.float-input:focus { border-color:#2563eb;background:white;box-shadow:0 0 0 4px rgba(37,99,235,0.1); }
.eye-btn { position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#9ca3af;background:none;border:none;cursor:pointer;padding:2px;transition:color 0.2s,transform 0.2s; }
.eye-btn:hover { color:#2563eb;transform:translateY(-50%) scale(1.15); }
.match-icon { position:absolute;right:40px;top:50%;transform:translateY(-50%); }

.error-banner { display:flex;align-items:center;gap:8px;background:#fef2f2;border:1px solid #fecaca;color:#dc2626;padding:10px 14px;border-radius:10px;font-size:0.85rem;font-weight:500; }
.success-banner { display:flex;align-items:center;gap:10px;background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;padding:12px 14px;border-radius:10px;font-size:0.85rem;font-weight:500; }

.btn-primary { position:relative;overflow:hidden;width:100%;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:white;font-weight:700;font-size:0.95rem;padding:13px;border-radius:12px;border:none;cursor:pointer;transition:transform 0.15s,box-shadow 0.15s,opacity 0.2s;box-shadow:0 4px 20px rgba(37,99,235,0.35); }
.btn-primary:hover:not(:disabled) { transform:translateY(-2px);box-shadow:0 8px 28px rgba(37,99,235,0.45); }
.btn-primary:active:not(:disabled) { transform:translateY(0); }
.btn-primary:disabled { opacity:0.65;cursor:not-allowed; }

.card-enter-active { animation:card-appear 0.7s cubic-bezier(0.34,1.56,0.64,1); }
@keyframes card-appear { from{transform:translateY(40px) scale(0.95);opacity:0}to{transform:translateY(0) scale(1);opacity:1} }

.fade-quick-enter-active,.fade-quick-leave-active { transition:opacity 0.15s,transform 0.15s; }
.fade-quick-enter-from { opacity:0;transform:scale(0.8); }
.fade-quick-leave-to { opacity:0;transform:scale(0.8); }

.shake-enter-active { animation:shake 0.5s ease; }
@keyframes shake { 0%,100%{transform:translateX(0)}20%{transform:translateX(-8px)}40%{transform:translateX(8px)}60%{transform:translateX(-5px)}80%{transform:translateX(5px)} }

.slide-down-enter-active { animation:slide-down 0.3s ease; }
.slide-down-leave-active { animation:slide-down 0.2s ease reverse; }
